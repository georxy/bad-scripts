...
def avg_analysis_speed(time_start, time_end, f_name):
    file_test = desktop + "/" + f_name + ".csv"
    df = pd.read_csv(file_test)
    sites_analyzed = len(df)
    time_spent = time_end - time_start
    avg_speed = round(time_spent/sites_analyzed, 2)
    spd_text = f"avg time spent on 1 website is {avg_speed}s"
    ...
    hide_btt = Button(root, text="Hide", command=hide_btt, font=("Arial", 12), width=15)
    hide_btt.grid(row=1, column=1)
...
def filter_functions():
    ...
    def click_blog():
        ...
        t2 = time.time()
        avg_analysis_speed(t1, t2, f_name)
...
def ai_bulk_functions():
    ...
    def header_bulk():
        ...
        t2 = time.time()
        avg_analysis_speed(t1, t2, file_name)
...
def dev_functions():
    ...
    entry_pass = Entry(borderwidth=6, font=("Arial", 14), width=20)
    ...
    def btt_pass_hide():
        entry_pass.destroy()
        btt_pass.destroy()
        btt_pass_hide.destroy()

    def read_pass():
        password = entry_pass.get()
        _password = date.today().strftime("%d/%m/%Y")
        if password == _password:
            ai_button.grid(row=6, column=0)
            bai_button.grid(row=7, column=0)
    btt_pass = Button(root, text="Enter Password", command=read_pass, font=("Arial", 12), width=25)
    btt_pass.grid(row=1, column=1)

    btt_pass_hide = Button(root, text="Hide", command=btt_pass_hide, font=("Arial", 12), width=25)
    btt_pass_hide.grid(row=2, column=1)


dev_button = Button(root, text="Developer Mode", command=dev_functions, font=("Arial", 12), width=25)
dev_button.grid(row=5, column=0)
